...
pub type UnicodeBlocksGroupedMap = Vec<Block>;
...
pub fn grouped_by(gvec: &[GlyphRef], blocks: &UnicodeBlocksMap) -> UnicodeBlocksGroupedMap {
    ...
    blocks.iter().enumerate().for_each(|(i, key_name)| {
        ...
        let grs = gvec
            ...
            .filter(|gr| {
                gr.unicode
                    .iter()
                    .any(|u| u.encoding >= *low && u.encoding <= *high)
            })
            .map(|v| v.to_owned())
            .collect();
        grouped_by.insert(
    ...
    grouped_by.push(Block {
        range: None,
        name: "Unencoded",
        glyphs: gvec
            .iter()
            .filter(|gr| gr.unicode.len() == 0)
            .map(|v| v.to_owned())
            .collect(),
    });
    ...
    fn log(&self) {
        ...
        for (k, v) in self.iter().enumerate() {
            log::debug!(
                ...
                k + 1,
                v,
                v.glyphs.iter().map(|vv| format!("{}", vv)).join(", ")
            );
        }
    }
}
